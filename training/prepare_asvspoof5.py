...
Usage:
    python -m training.prepare_asvspoof5 --dataset-dir <PATH>

What it does:
    1. Verifies required protocol files exist.
...
def main() -> None:
    ...
    proto_dir = dataset_dir / "protocols"
    required_protocols = [
        "ASVspoof5.train.tsv",
    ...
    for name in required_protocols:
        ok = _check_protocol(proto_dir / name)
        all_ok = all_ok and ok
    for name in optional_protocols:
        _check_protocol(proto_dir / name)
    ...
    train_audio_dir = dataset_dir / "train" / "flac_T"
    dev_audio_dir   = dataset_dir / "dev"   / "flac_D"

    n_train_flac = _count_flac(train_audio_dir)
    n_dev_flac   = _count_flac(dev_audio_dir)
    logger.info(f"  Train audio [{train_audio_dir}]: {n_train_flac:,} .flac files")
    logger.info(f"  Dev audio   [{dev_audio_dir}]:   {n_dev_flac:,} .flac files")

    if n_train_flac == 0:
        logger.error("No training audio files found. Check dataset extraction.")
        sys.exit(1)
    ...
    summary = {
        ...
        "train": t_stats,
        "val": v_stats,
        "train_flac_files": n_train_flac,
        "dev_flac_files": n_dev_flac,
    ...
    logger.info("  Next: python -m training.train --asvspoof5-dir <DATASET>")
